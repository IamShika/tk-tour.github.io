...
class MapEngine {
    constructor(containerId, options = {}) {
        ...
        this._workerUrl = null; // Cloudflare Worker proxy URL (Bug 1)
        this._3dMode = false; // 2D/3D toggle state (Bug 3)
    ...
    addStreetViewLayer(geojson, onClickCallback) {
        if (this.map.getSource('streetview-source')) {
            ...
            // The visible dots for each panorama
            this.map.addLayer({
            ...
            // Bug 2 Fix: Invisible hit-area layer with larger radius for mobile touch targets (added AFTER visible dots so it's on top)
            this.map.addLayer({
                id: 'streetview-points-hitarea',
                type: 'circle',
                source: 'streetview-source',
                paint: {
                    'circle-radius': 24,
                    'circle-color': 'rgba(0,0,0,0)', // Transparent color ensures it registers clicks
                    'circle-stroke-width': 0
                },
                filter: ['==', '$type', 'Point']
            });

            // Use hit-area layer for cursor + click so mobile gets the larger touch target
            this.map.on('mouseenter', 'streetview-points-hitarea', () => {
                this.map.getCanvas().style.cursor = 'pointer';
            });

            this.map.on('mouseleave', 'streetview-points-hitarea', () => {
                this.map.getCanvas().style.cursor = '';
            });

            this.map.on('click', 'streetview-points-hitarea', (e) => {
    ...
    addPolygon(id, coords, options = {}) {
        ...
        this.map.addSource(sourceId, { 
            type: 'geojson', 
            data: geojson,
            promoteId: 'featureId' // Required for feature-state with string IDs
        });
        
        const isHoverable = !!options.interactiveHover;
        ...
        this.map.addLayer({
            ...
            paint: {
                ...
                'fill-opacity': isHoverable ? 
                    ['case', ['boolean', ['feature-state', 'hover'], false], (options.fillOpacity || 0.4), 0] 
        ...
        this.map.addLayer({
            ...
            paint: {
                ...
                'line-opacity': isHoverable ? 
                    ['case', ['boolean', ['feature-state', 'hover'], false], (options.opacity || 0.9), 0] 
                    : (options.opacity || 0.9)
            }
        });
        ...
        // Always add mouse enter/leave for interactiveHover, and merge with options.onMouseOver
        this.map.on('mouseenter', fillLayerId, (e) => {
            this.map.getCanvas().style.cursor = 'pointer';
            if (isHoverable) {
                this.map.setFeatureState({ source: sourceId, id: id }, { hover: true });
            }
            if (options.onMouseOver) options.onMouseOver(e);
        });
        
        this.map.on('mouseleave', fillLayerId, (e) => {
            this.map.getCanvas().style.cursor = '';
            if (isHoverable) {
                this.map.setFeatureState({ source: sourceId, id: id }, { hover: false });
            }
            if (options.onMouseOut) options.onMouseOut(e);
        });
    ...
    removePolygon(id) {
        [`poly-fill-${id}`, `poly-outline-${id}`, `poly-label-${id}`].forEach(layerId => {
            if (this.map.getLayer(layerId)) this.map.removeLayer(layerId);
        });
        const sourceId = `poly-src-${id}`;
        if (this.map.getSource(sourceId)) this.map.removeSource(sourceId);
    ...
    // Bug 1 Fix: Cloudflare Worker proxy URL — routes API requests server-side so keys never reach the browser
    setWorkerUrl(url) {
        this._workerUrl = url;
        console.log('[MapEngine] Worker proxy URL set:', url);
    }
    ...
    // Bug 3 Fix: 2D/3D mode toggle — used by DevTools and the layer switcher
    // When disabling 3D: snap to pitch 0 AND lock pitch gestures so the user
    // cannot tilt the map back manually.
    set3DMode(enabled) {
        this._3dMode = !!enabled;
        if (enabled) {
            // Unlock pitch gestures
            this.map.touchPitch.enable();
            this.map.dragRotate.enable();
        } else {
            // Snap flat and lock gestures
            this.map.easeTo({ pitch: 0, bearing: 0, duration: 400 });
            // Small delay so the easing doesn't fight the disable
            setTimeout(() => {
                this.map.touchPitch.disable();
                this.map.dragRotate.disable();
            }, 420);
        }
        console.log('[MapEngine] 3D mode:', this._3dMode);
    }
    ...
    enterDrawingMode() {
        this._previous3dMode = this._3dMode;
        if (this._3dMode) {
    ...
    exitDrawingMode() {
        if (this._previous3dMode !== undefined && this._previous3dMode !== this._3dMode) {
            this.set3DMode(this._previous3dMode);
        }
    }
    ...
    async _createGoogleSession(mapType = 'satellite') {
        ...
        try {
            ...
            if (this._workerUrl) {
                // Bug 1 Fix: Route through Cloudflare Worker proxy — API key stays server-side
                const res = await fetch(`${this._workerUrl}/google/session?mapType=${mapType}`);
                if (!res.ok) {
                    const errText = await res.text();
                    throw new Error(`Worker session endpoint returned ${res.status}: ${errText}`);
                }
                data = await res.json();
            } else {
                // Legacy: direct API call (local dev only)
                if (!this._googleApiKey) throw new Error('No Google Maps API key or worker URL configured');
                const res = await fetch(`https://tile.googleapis.com/v1/createSession?key=${this._googleApiKey}`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ mapType: mapType, language: 'th', region: 'TH' })
                });
                if (!res.ok) {
                    const errText = await res.text();
    ...
    async _ensureGoogleSatelliteLayer() {
        ...
        // Allow when using worker proxy (no local key needed)
        if (!this._googleApiKey && !this._workerUrl) {
            console.warn('[MapEngine] Cannot add Google satellite layer: no API key or worker URL set');
            return false;
        }
        ...
        // Bug 1 Fix: Use worker proxy URL when available, otherwise fall back to direct (local dev)
        const tileUrl = this._workerUrl
            ? `${this._workerUrl}/google/tiles/{z}/{x}/{y}?session=${session}`
            : `https://tile.googleapis.com/v1/2dtiles/{z}/{x}/{y}?session=${session}&key=${this._googleApiKey}`;
    ...
    async _ensureGoogleRoadmapLayer() {
        ...
        // Allow when using worker proxy (no local key needed)
        if (!this._googleApiKey && !this._workerUrl) {
            console.warn('[MapEngine] Cannot add Google roadmap layer: no API key or worker URL set');
            return false;
        }
        ...
        // Bug 1 Fix: Use worker proxy URL when available
        const tileUrl = this._workerUrl
            ? `${this._workerUrl}/google/tiles/{z}/{x}/{y}?session=${session}`
            : `https://tile.googleapis.com/v1/2dtiles/{z}/{x}/{y}?session=${session}&key=${this._googleApiKey}`;
    ...
    _ensureGistdaLayer() {
        ...
        // Bug 1 Fix: Allow GISTDA through worker proxy even without a local API key
        if (!this._gistdaApiKey && !this._workerUrl) return false;
        this._gistdaLayerAdded = true;

        try {
            // Use worker proxy URL when available, otherwise use direct key
            const tileUrl = this._workerUrl
                ? `${this._workerUrl}/gistda/tiles/{z}/{x}/{y}`
    ...
    async switchBaseLayer(layerType) {
        ...
        if (layerType === 'satellite') {
            ...
            const tryGistda = () => {
                if (this._gistdaApiKey || this._workerUrl) {
            ...
            if (this._mapProvider === 'google' && (this._googleApiKey || this._workerUrl)) {
